...
export default function AddMedicalTestModal({
  ...
  useEffect(() => {
    if (isOpen) {
      setName("");
      setDescription("");
      setIduom(uoms.length > 0 ? uoms[0].id : 0);
      setIdcategory(testCategories.length > 0 ? testCategories[0].id : 0);
      setNormalmin(0);
      setNormalmax(0);
      setIsCapsLock(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!name.trim() || iduom === 0 || idcategory === 0) return;

    setIsSubmitting(true);
    try {
      await onAdd(name, description, iduom, idcategory, normalmin, normalmax);
      onClose();
    } catch (error) {
      console.error(error);
    } finally {
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 text-white font-semibold">
            ...
            <span>Add New Medical Test</span>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            >
              {isSubmitting ? "Adding..." : "Add Medical Test"}
            </button>
            <button
              type="button"
              onClick={onClose}
