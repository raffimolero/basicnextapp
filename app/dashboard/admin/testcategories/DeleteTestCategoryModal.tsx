...
export default function DeleteTestCategoryModal({
  isOpen,
  onClose,
  onDelete,
  testCategory,
  ...
  const handleDelete = async () => {
    ...
    const confirmed = await ConfirmModal(
      `Are you sure you want to delete test category "${testCategory.name}"?`,
      {
        okText: "Yes, Delete",
        cancelText: "Cancel",
    ...
    setIsDeleting(true);
    try {
      await onDelete(testCategory.id);
      onClose();
    } catch (error) {
      console.error(error);
    } finally {
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 text-white font-semibold">
            ...
            <span>Delete Test Category</span>
          </div>
          <button
            onClick={onClose}
