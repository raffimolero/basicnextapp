...
export default function EditTestCategoryModal({
  isOpen,
  onClose,
  onEdit,
  testCategory,
}: EditTestCategoryModalProps) {
  ...
  useEffect(() => {
    if (isOpen && testCategory) {
      setName(testCategory.name);
      setDescription(testCategory.description);
      setIsCapsLock(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!testCategory || !name.trim()) return;

    setIsSubmitting(true);
    try {
      await onEdit(testCategory.id, name, description);
      onClose();
    } catch (error) {
      console.error(error);
    } finally {
  ...
  if (!isOpen || !testCategory) return null;
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div className="space-y-1">
            ...
            <p className="text-xs text-gray-500">
              Test category names are typically short abbreviations.
            </p>
          </div>
          ...
          <div className="space-y-1">
            ...
            <textarea
              ...
              placeholder="Describe this category of medical tests..."
              value={description}
