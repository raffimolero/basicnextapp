...
export default function Page() {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editing) {
        await updateUom(editing.id, name.trim(), description.trim());
        await showMessage("UOM updated.");
      } else {
        await addUom(name.trim(), description.trim());
        await showMessage("UOM added.");
      }
      resetForm();
      fetchUoms();
    } catch (error) {
      console.error(error);
      await showMessage("Failed to save UOM.");
    }
  };
    ...
    <PageGuardWrapper requiredRoles={["ADMINISTRATOR"]}>
      <div className="space-y-4">
        ...
        <div className="text-sm text-gray-700">
          Showing {filtered.length} of {uoms.length} units
        </div>
      </div>
    </PageGuardWrapper>
  );
